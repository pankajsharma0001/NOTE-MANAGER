  ...
  return (
    <div>
      {loading ? (
        ...
        <p className="text-center text-gray-400">No pending notes.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {pendingNotes.map(note => (
            <div key={note._id} className="p-4 bg-gray-800 rounded-lg shadow-lg hover:bg-gray-700 transition">
              <div className="flex items-center mb-2 cursor-pointer hover:underline" onClick={() => setProfileModal(note.uploadedBy)}>
                <Image src={note.uploadedBy?.image || "/default-avatar.png"} alt={note.uploadedBy?.name || "User"} width={40} height={40} className="w-10 h-10 rounded-full object-cover mr-3" />
      ...
      {profileModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 sm:p-0">
          <div className="bg-gray-900 rounded-xl p-6 w-full max-w-xs sm:max-w-sm relative cursor-move"
            ...
            <div className="flex flex-col items-center text-center">
              <Image src={profileModal.image || "/default-avatar.png"} alt={profileModal.name || "User"} width={100} height={100} className="w-24 h-24 rounded-full object-cover mb-4" />
      ...
      {selectedNote && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 sm:p-0">
          <div className="bg-gray-900 p-4 sm:p-6 rounded-xl w-full max-w-xs sm:max-w-md relative overflow-auto max-h-[90vh]">
            <button className="absolute top-2 right-2 text-gray-400 hover:text-white" onClick={() => { setSelectedNote(null); setEditMode(false); }}>✖</button>
            ...
            {editMode ? (
              <div className="flex flex-col gap-3">
                ...
                <select value={editedNote.semester || ""} onChange={e => setEditedNote({ ...editedNote, semester: e.target.value, subject: "" })}
                  className="p-2 rounded bg-gray-800 text-white border border-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 w-full">
                ...
                <select value={editedNote.subject || ""} onChange={e => setEditedNote({ ...editedNote, subject: e.target.value })}
                  className="p-2 rounded bg-gray-800 text-white border border-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 w-full" disabled={!editedNote.semester}>
                ...
                <textarea value={editedNote.content} onChange={e => setEditedNote({ ...editedNote, content: e.target.value })}
                  className="p-2 rounded bg-gray-800 text-white border border-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 w-full" rows={4} />
                ...
                <label className="cursor-pointer bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded inline-block text-white">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={e => setEditedNote({ ...editedNote, newFile: e.target.files[0] })} />
                </label>
